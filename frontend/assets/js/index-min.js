...
jQuery(function($){
    ...
    slideshows =['industria' ,  'terceario' , 'fabril' , 'metalomecanica'];

    for(var index in slideshows){

        if(jQuery(".main-slideshow-"+slideshows[index]+" .hp-slide").length > 1){
            var hpslideshow = jQuery(".main-slideshow-"+slideshows[index]).owlCarousel({
            items :1,
            loop  : true,
            autoplay:false,
            autoplayHoverPause:true,
            ...
            autoplayTimeout:4000,
            nav:true,
            navContainer:'#nav-crtl-'+slideshows[index],
            dotsContainer:'#nav-dot-'+slideshows[index],
            dots:false
          });
    ...
    function initialize() {
        ...
        var myLatLng  = new google.maps.LatLng(38.7845822,-9.3221402);
        ...
        var contentBox = $(
            ...
            '<ul>'+
            '<li><span>M.</span>  <span>SOMETIM SA <br/> Rua de Sao joao</span></li>'+
            '<li><span>T.</span>  00351 334 55 55</li>'+
...
(function($ , window){
    ...
    MenuSlide.prototype = {
        ...
        onloadSelect: function(name){
            ...
            $('html, body').animate({scrollTop:$(section[0].section ).offset().top - self.settings.compensation}, 'slow' , function(){self.hasClick = false;});
